...
def seven(input):
	count = 0 
	vowel_count = list(input.lower())
	a = len(vowel_count)
	#create list to loop through vowel_count
	for vowels in range(a):
		if vowel_count[vowels] == 'a' or vowel_count[vowels] == "e" or vowel_count[vowels] == "i" or vowel_count[vowels] == "o" or vowel_count[vowels] == "u":
			count += 1
	return count
...
def eight(input):
	...
	if input > 10 or input < 1:
		print("invalid number, please enter an input between 1-10")
	else:
		count = 1
		while input > count:
			factorial_list.append(count + 1)
			count += 1
		factorial = 1
